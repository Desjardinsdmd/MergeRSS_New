...
import { inferTag, whatHappened, whyItMatters, signalLevelStyle, deduplicateItems } from './intelligenceUtils';
...
export default function TopFiveToday({ feedIds, feeds }) {
    ...
    const { data: items = [], isLoading } = useQuery({
        ...
        queryFn: async () => {
            ...
            // Deduplicate with source diversity enforcement, take top 5
            return deduplicateItems(raw, feedMap).slice(0, 5);
        },
    ...
    if (!items.length) {
        return (
            <div className="bg-stone-900 border border-stone-800 p-5">
                ...
                <p className="text-stone-600 text-sm">No high-signal items from the last 48h yet.</p>
            </div>
        );
    }
    ...
    return (
        <div className="bg-stone-900 border border-stone-800">
            <div className="flex items-center gap-2 px-5 py-4 border-b border-stone-800">
                <Zap className="w-4 h-4 text-[hsl(var(--primary))]" />
                <h2 className="text-sm font-semibold text-stone-300 uppercase tracking-wider">Top 5 Today</h2>
                <span className="text-xs text-stone-600 ml-auto">AI-ranked · source-diverse</span>
            </div>
            <div className="divide-y divide-stone-800/80">
                {items.map((item, idx) => {
                    ...
                    return (
                        <div
                            key={item.id}
                            className={`px-5 py-4 hover:bg-stone-800/40 transition-colors cursor-pointer ${isHighSignal ? 'border-l-2 border-[hsl(var(--primary))]/60' : ''}`}
                        ...
                        >
                            <div className="flex items-start gap-3">
                                ...
                                <div className="flex-1 min-w-0">
                                    {/* Headline */}
                                    <h3 className={`text-sm font-semibold leading-snug mb-1.5 ${isHighSignal ? 'text-stone-50' : 'text-stone-100'}`}>
