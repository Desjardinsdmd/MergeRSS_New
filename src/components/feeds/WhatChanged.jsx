...
import { inferTag, whatHappened, signalLevelStyle } from './intelligenceUtils';
...
export default function WhatChanged({ feedIds = [], feeds = [] }) {
    ...
    // Record current visit timestamp, retrieve last visit
    const lastVisit = useMemo(() => {
        const stored = localStorage.getItem(LAST_VISIT_KEY);
        return stored ? new Date(stored) : new Date(Date.now() - 8 * 60 * 60 * 1000); // default: 8h ago
    }, []);
    ...
    const { data: newItems = [], isLoading } = useQuery({
        ...
        queryFn: async () => {
            ...
            return (raw || [])
                .filter(i => (i.importance_score ?? 0) >= 40 || i.intelligence_tag === 'Risk' || i.intelligence_tag === 'Opportunity')
    ...
    if (isLoading || !newItems.length) return null;
                    ...
                    return (
                        <div key={item.id} className="px-5 py-3.5 hover:bg-stone-800/30 transition-colors">
                            ...
                            <a
                                ...
                                className="group flex items-start gap-2 mb-1.5"
                            >
                            ...
                            {/* What happened */}
                            {happened && (
                                <p className="text-xs text-stone-400 leading-snug mb-2 line-clamp-1">{happened}</p>
                            )}
