 ...
 *
 * Driving it:
 *   - Each call works for up to 45s, then records its cursor in SystemHealth
 *     (job_type 'article_migration') and tries to invoke the next hop itself.
 *   - { resume: true } picks up from the last recorded cursor. A scheduled workflow
 *     calls this every 2 minutes as a safety net in case the self-invoke chain drops.
 *   - A 'running' SystemHealth record acts as a lease so two hops never overlap
 *     (overlap could create the same Article twice).
 *
 * Admin only.
 */
...
Deno.serve(async (req) => {
    ...
    // Lease: never run two hops at once
    const running = extractItems(await svc.SystemHealth.filter({ job_type: JOB, status: 'running' }, '-created_date', 1))[0];
    if (running && Date.now() - new Date(running.created_date).getTime() < LEASE_MS) {
        return Response.json({ skipped: 'another hop is running' });
    }

    // Resolve starting point
    let days = Number(body.days) > 0 ? Number(body.days) : 90;
    let hop = body.hop || 0;
    let subIndex = body.cursor?.sub_index || 0;
    let skip = body.cursor?.skip || 0;
    let cutoff = body.cutoff || null;
    if (body.resume) {
        const last = extractItems(await svc.SystemHealth.filter({ job_type: JOB, status: 'completed' }, '-created_date', 1))[0];
        if (last?.metadata?.done) return Response.json({ skipped: 'migration already complete', last: last.metadata });
        if (last?.metadata?.cursor) {
            subIndex = last.metadata.cursor.sub_index || 0;
            skip = last.metadata.cursor.skip || 0;
            hop = (last.metadata.hop || 0) + 1;
            days = last.metadata.days || days;
            cutoff = last.metadata.cutoff || cutoff;
        }
    }
    if (!cutoff) cutoff = new Date(Date.now() - days * 86400_000).toISOString();

    const lease = await svc.SystemHealth.create({
        job_type: JOB, status: 'running', started_at: new Date(t0).toISOString(),
        metadata: { hop, cursor: { sub_index: subIndex, skip } },
    }).catch(() => null);

    const subs = extractItems(await svc.Subscription.filter({}, 'created_date', 5000))
        .filter(s => s.legacy_feed_id && s.source_id);
    if (!subs.length) {
        if (lease) await svc.SystemHealth.delete(lease.id).catch(() => {});
    ...
    const metadata = { ...stats, hop, days, cutoff, done, error, cursor: { sub_index: subIndex, skip }, subscriptions: subs.length };
    await svc.SystemHealth.create({
        job_type: JOB, status: error ? 'failed' : 'completed',
        started_at: new Date(t0).toISOString(), completed_at: new Date().toISOString(),
        error_message: error || undefined, metadata,
    }).catch(() => {});
    if (lease) await svc.SystemHealth.delete(lease.id).catch(() => {});

    // Fast path: chain the next hop. The scheduled resume run covers it if this drops.
    if (outOfTime && hop < MAX_HOPS) {
        try {
            await Promise.race([
                base44.asServiceRole.functions.invoke('migrateArticles', { resume: true }),
                new Promise(r => setTimeout(r, 1500)),
            ]);
        } catch { /* scheduled resume will pick it up */ }
    }
